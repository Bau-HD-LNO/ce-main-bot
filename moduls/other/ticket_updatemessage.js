...
module.exports = client => {
  ...
  var job3 = new CronJob('0 */15 * * * *', function () {
    client.channels.fetch(settings.ticket_umsg.chid).then(ch => {
      ch.messages.fetch(settings.ticket_umsg.msgid).then(async msg => {
        let allmembers = await msg.guild.members.fetch();
        let onlinesupporters = [
          ...allmembers.filter(m => !m.user.bot && m.roles.highest.rawPosition >= msg.guild.roles.cache.get(settings.ticket_umsg.supportid).rawPosition)
          .filter(m => m.presence)
          .values()
        ]
